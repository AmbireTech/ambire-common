...
const test_rrsets = [
  ...
  [
    ethers.hexlify(ethers.toUtf8Bytes('xyz.')),
  ...
  [
    ethers.hexlify(ethers.toUtf8Bytes('ethlab.xyz.')),
  ...
  [
    ethers.hexlify(ethers.toUtf8Bytes('_ens.ethlab.xyz.')),
...
async function verifySubmission(instance: any, rrsets: any) {
  var response = await instance.verifyRRSet(rrsets)
}

async function verifyFailedSubmission(instance: any, rrsets: any) {
...
//       instance.setDigest(1, signer.address, { from: signer.address }),
//     )
//   })
...
//     var instance = await deployContract()
//     await expectRevert.unspecified(
//       instance.setAlgorithm(1, signer.address, { from: signer.address }),
//     )
//   })
