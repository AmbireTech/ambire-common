...
import { wallet, addressOne, AmbireAccountFactory, AmbireAccount, expect, buildInfo, deploySalt, deployGasLimit, assertion } from "./config"
...
function getAmbireAccountAddress(factoryAddress: string, bytecode: string) {
  return ethers.getCreate2Address(factoryAddress, ethers.toBeHex(deploySalt, 32), ethers.keccak256(bytecode))
}
