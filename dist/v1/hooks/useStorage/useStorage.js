...
const setInitDefault = (item) => item;
/**
 ...
 */
function useStorage({ storage, key, defaultValue = null, isStringStorage = false, 
// @ts-ignore FIXME: Figure out why TypeScript complains
setInit = setInitDefault }) {
    const [item, set] = (0, react_1.useState)(() => {
        ...
        // so that TypeScript doesn't complain
        if (isStringStorage)
            return setInit(storage.getItem(key));
    ...
    const setItem = (0, react_1.useCallback)((value) => {
        set((prevState) => {
