...
async function parseEmail(email) {
    const dkims = (0, parse_1.parse)(email).dkims.map((dkim) => {
        ...
        const bodyHash = (0, crypto_1.createHash)(algorithm).update(dkim.processedBody).digest();
        const bodyHashMatched = bodyHash.compare(dkim.signature.hash) !== 0;
        if (bodyHashMatched) {
            throw new Error('body hash did not verify');
        }
        const hash = (0, crypto_1.createHash)(algorithm).update(dkim.processedHeader).digest();
        return {
            ...dkim,
            hash
        };
