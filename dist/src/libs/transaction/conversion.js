...
const handleFiatToTokenConversion = (amount, amountFormatted, tokenPrice, fromSelectedToken) => {
    ...
    const tokenAmount = formatUnits((amountInFiatBigInt * CONVERSION_PRECISION_POW) / tokenPriceBigInt, amountInFiatDecimals + CONVERSION_PRECISION - tokenPriceDecimals);
    return { tokenAmount, fiatAmount: amount };
};
const handleTokenToFiatConversion = (amount, amountFormatted, tokenPrice, fromSelectedToken) => {
    ...
    const { tokenPriceBigInt, tokenPriceDecimals } = convertTokenPriceToBigInt(Number(tokenPrice));
    const fiatAmount = formatUnits(formattedAmount * tokenPriceBigInt, fromSelectedToken.decimals + tokenPriceDecimals);
    return { tokenAmount: amount, fiatAmount };
};
export const handleAmountConversion = (amount, amountFormatted, fromSelectedToken, isInFiatMode, hardCodedCurrency) => {
    if (amount === '') {
