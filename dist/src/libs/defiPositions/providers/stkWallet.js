...
export function getStakedWalletPositions(stkWallet) {
    if (!stkWallet || !stkWallet.amount)
        return null;
    const positionInUSD = getAssetValue(BigInt(stkWallet.amount), Number(stkWallet.decimals), stkWallet.priceIn);
    const positions = [
        {
            id: 'stk-wallet',
            additionalData: {
