...
const addRPCError = (errors, chainId, networks) => {
    ...
    const errorId = hasMultipleRpcUrls
    ...
    if (existingError) {
        ...
        const text = 'Affected features: visible assets, DeFi positions, sign message/transaction, ENS domain resolving, add account.';
        let title = '';
        let actions;
...
const addPortfolioError = (errors, networkName, newError) => {
    ...
    if (existingError) {
        if (!existingError.networkNames.includes(networkName)) {
            ...
            if (lastIndexOfOn !== -1) {
                existingError.title = `${existingError.title.substring(0, lastIndexOfOn)} on ${networkNames.join(', ')}`;
            }
            else {
...
const getNetworkName = (networks, chainId) => {
    ...
    if (chainId === 'gasTank')
        ...
        networkName = 'Rewards';
    else if (chainId === 'projectedRewards')
...
const getNetworksWithDeFiPositionsErrorErrors = ({ networks, portfolioState, providers, networksWithPositions }) => {
    ...
    Object.keys(portfolioState).forEach((chainId) => {
        ...
        if ((typeof lastSuccessfulUpdate === 'number' &&
        ...
        const providerNamesWithErrors = defiState.providerErrors
            ?.filter(({ providerName }) => {
