...
function geckoResponseIdentifier(tokenAddr, networkId) {
    return coingecko_1.geckoIdMapper(tokenAddr, networkId) || tokenAddr.toLowerCase();
}
...
function geckoRequestBatcher(queue) {
    ...
    for (const queueItem of queue) {
        ...
        const geckoId = coingecko_1.geckoIdMapper(queueItem.data.address, queueItem.data.networkId);
        if (geckoId)
            segmentId += ':natives';
        else
    ...
    const pages = Object.entries(segments)
        .map(([key, queueSegment]) => pagination_1.paginate(queueSegment, BATCH_LIMIT).map((page) => ({ key, queueSegment: page })))
        .flat(1);
    ...
    return pages.map(({ key, queueSegment }) => {
        ...
        const geckoPlatform = coingecko_1.geckoNetworkIdMapper(queueSegment[0].data.networkId);
        // @TODO API key, customizable
        const apiKeyString = ''; //`x_cg_pro_api_key=`
        let url;
        if (key.endsWith('natives'))
            url = `https://api.coingecko.com/api/v3/simple/price?ids=${dedup(queueSegment.map((x) => coingecko_1.geckoIdMapper(x.data.address, x.data.networkId))).join('%2C')}&vs_currencies=${baseCurrency}${apiKeyString}`;
        else
