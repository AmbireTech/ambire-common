...
export function geckoResponseIdentifier(tokenAddr, network) {
    return geckoIdMapper(tokenAddr, network) || tokenAddr.toLowerCase();
}
export function geckoRequestBatcher(queue) {
    const segments = {};
    ...
    const pages = Object.entries(segments)
        .map(([key, queueSegment]) => paginate(queueSegment, BATCH_LIMIT).map((page) => ({ key, queueSegment: page })))
        .flat(1);
    ...
    return pages.map(({ key, queueSegment }) => {
        ...
        if (key.endsWith('natives'))
            url = `${mainApiUrl}/api/v3/simple/price?ids=${dedup(queueSegment.map((x) => geckoIdMapper(x.data.address, x.data.network))).join('%2C')}&vs_currencies=${baseCurrency}`;
        else
