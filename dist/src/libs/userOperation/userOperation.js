...
import { AMBIRE_ACCOUNT_FACTORY, AMBIRE_PAYMASTER, AMBIRE_PAYMASTER_SIGNER, ENTRY_POINT_MARKER, ERC_4337_ENTRYPOINT } from '../../consts/deploy';
...
export function getRequestType(accountState) {
    return accountState.isDeployed && !accountState.isErc4337Enabled ? 'activator' : 'standard';
}
export function shouldUseOneTimeNonce(accountState) {
    return getRequestType(accountState) !== 'standard';
}
export function getUserOperation(account, accountState, accountOp, bundler, entryPointSig) {
    const userOp = {
...
export function isErc4337Broadcast(acc, network, accountState) {
...
export function shouldAskForEntryPointAuthorization(network, account, accountState, alreadySigned) {
    if (alreadySigned)
        return false;
    return (account.creation &&
        account.creation.factoryAddr === AMBIRE_ACCOUNT_FACTORY &&
