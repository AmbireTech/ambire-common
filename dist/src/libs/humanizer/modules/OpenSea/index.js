...
const parsePrice = (price, numerator, denumerator) => BigInt((price * numerator) / denumerator);
const parseOrder = (order) => {
    const [params, num, denum
    // data2, data3
    ] = order;
    const [, , 
    // currentOwner
    ...
    const items = offers.map((o) => {
        const [, 
        // type
        address, id, fromAmount, endAmount] = o;
        return {
            address,
            id,
    ...
    consideration.forEach((o) => {
        const [type, token, tokenId, , 
        // fromAmount
        endAmount] = o;
        if (type === 0n || type === 1n)
            tokenPayments[token] = (tokenPayments[token] || 0n) + parsePrice(endAmount, num, denum);
        if (type === 2n || type === 3n)
            payment.push({ address: token, amountOrId: BigInt(tokenId) });
    });
    ...
    return irCalls.map((call) => {
        if ([
            ...
            if (call.data.slice(0, 10) === iface.getFunction('fulfillBasicOrder').selector)
                ...
                orders = iface.decodeFunctionData('fulfillBasicOrder_efficient_6GL6yc', call.data);
            const data = orders.map((i) => {
                const [considerationToken, considerationIdentifier, considerationAmount, , , 
                // offerer,
                // zone,
        ...
        if (call.data.startsWith(iface.getFunction('fulfillAvailableAdvancedOrders').selector)) {
            ...
            if (totalOrders.length > 30)
                totalOrders = dedupe1155Orders(totalOrders);
            // still not deduped
            if (totalOrders.length > 30)
                return {
                    ...call,
