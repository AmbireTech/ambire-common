...
import { ensMessageModule, entryPointModule, erc20Module, erc721Module, legendsMessageModule, openseaMessageModule, permit2Module } from './messageModules';
...
const humanizeAccountOp = (_accountOp, options) => {
    ...
    humanizerCallModules.forEach((hm) => {
        try {
            currentCalls = hm(accountOp, currentCalls, humanizerInfo, humanizerOptions);
        }
        catch (error) {
            console.error(error);
