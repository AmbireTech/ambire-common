// @ts-nocheck
import { AbiCoder, concat, encodeRlp, getAddress, getBytes, hashMessage, hexlify, Interface, isHexString, keccak256, toBeHex, toNumber, toUtf8Bytes } from 'ethers';
...
import { accountOpSignableHash, callToTuple, getSignableHash } from '../accountOp/accountOp';
 ...
 */
export const wrapUnprotected = (signature) => {
    return `${signature}00`;
};
/**
 ...
 */
export const wrapStandard = (signature) => {
    return `${signature}01`;
};
/**
...
export const getAmbireReadableTypedData = (chainId, verifyingAddr, v1Execute) => {
    const domain = {
        name: 'Ambire',
        version: '1',
 ...
 */
export const getTypedData = (chainId, verifyingAddr, msgHash) => {
    const domain = {
        name: 'Ambire',
        version: '1',
        chainId: chainId.toString(),
 ...
 * Return the typed data for EIP-712 sign
 */
export const get7702UserOpTypedData = (chainId, txns, packedUserOp, userOpHash) => {
 ...
 */
export const wrapCounterfactualSign = (signature, creation) => {
 ...
 */
export async function verifyMessage({ provider, signer, signature, message, authorization, typedData }) {
    let finalDigest;
    if (message) {
        try {
        ...
        try {
            ...
            if ('AmbireReadableOperation' in typedData.types) {
                ...
                finalDigest = hexlify(getSignableHash(ambireReadableOperation.addr, ambireReadableOperation.chainId, ambireReadableOperation.nonce, ambireReadableOperation.calls.map(callToTuple)));
            }
            else {
                // TODO: Hardcoded to V4, use the version from the typedData if we want to support other versions?
                finalDigest = hexlify(TypedDataUtils.eip712Hash(adaptTypedMessageForMetaMaskSigUtil({ ...typedData }), SignTypedDataVersion.V4));
            }
            if (!finalDigest)
...
export async function getExecuteSignature(network, accountOp, accountState, signer) {
    ...
    // in eip-712 format
    const typedData = getTypedData(network.chainId, accountState.accountAddr, hexlify(accountOpSignableHash(accountOp, network.chainId)));
    return wrapStandard(await signer.signTypedData(typedData));
}
export async function getPlainTextSignature(messageHex, network, account, accountState, signer, isOG = false) {
    ...
    if (!!account.safeCreation) {
        ...
        const typedData = getSafeTypedDataForIsValidSignature(network.chainId, account.addr, hashMessage(getBytes(messageHex)));
        return {
            signature: (await signer.signTypedData(typedData)),
            hash: `0x${TypedDataUtils.eip712Hash(adaptTypedMessageForMetaMaskSigUtil({ ...typedData }), SignTypedDataVersion.V4).toString('hex')}`
        };
    }
    if (!account.creation)
    ...
    if (!accountState.isV2) {
        ...
        return { signature: wrapUnprotected(await signer.signMessage(messageHex)) };
    }
    ...
    return { signature: wrapStandard(await signer.signTypedData(typedData)) };
}
export async function getEIP712Signature(message, account, accountState, signer, network, isOG = false) {
    ...
    if (!!account.safeCreation) {
        ...
        const typedData = getSafeTypedDataForIsValidSignature(network.chainId, account.addr, `0x${TypedDataUtils.eip712Hash(adaptTypedMessageForMetaMaskSigUtil({ ...message }), SignTypedDataVersion.V4).toString('hex')}`);
        return {
            signature: (await signer.signTypedData(typedData)),
            hash: `0x${TypedDataUtils.eip712Hash(adaptTypedMessageForMetaMaskSigUtil({ ...typedData }), SignTypedDataVersion.V4).toString('hex')}`
        };
    ...
    if (!accountState.isV2) {
        ...
        return { signature: wrapUnprotected(await signer.signTypedData(message)) };
    }
    ...
    if ('AmbireReadableOperation' in message.types) {
        ...
        const hash = hexlify(getSignableHash(ambireReadableOperation.addr, ambireReadableOperation.chainId, ambireReadableOperation.nonce, ambireReadableOperation.calls.map(callToTuple)));
        ...
        return { signature: wrapWallet(signature, account.addr) };
    }
    return { signature: wrapUnprotected(await signer.signTypedData(message)) };
}
// get the typedData for the first ERC-4337 deploy txn
export async function getEntryPointAuthorization(addr, chainId, nonce) {
...
export function adjustEntryPointAuthorization(entryPointSig) {
...
export function getAuthorizationHash(chainId, contractAddr, nonce) {
...
export function get7702Sig(chainId, nonce, implementation, signature) {
    return {
...
export function getAppFormatted(signature, account, accountState) {
    if (isHexString(signature))
        return getHexStringSignature(signature, account, accountState);
    return signature;
}
 ...
 */
export const toPersonalSignHex = (input) => {
    if (typeof input === 'string') {
        return isHexString(input) ? input : hexlify(toUtf8Bytes(input));
    }
    return hexlify(input);
};
export const getSafeTypedData = (chainId, safeAddr, safeTx) => {
    const domain = {
        chainId: chainId.toString(),
...
export const getSafeV1TypedData = (safeAddr, safeTx) => {
    const domain = {
        verifyingContract: safeAddr
    };
    const types = {
...
export const getSafeTypedDataForIsValidSignature = (chainId, safeAddr, hash) => {
    const domain = {
        chainId: chainId.toString(),
        verifyingContract: safeAddr
    };
