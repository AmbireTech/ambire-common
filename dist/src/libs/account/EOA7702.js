...
export class EOA7702 extends BaseAccount {
    ...
    getEstimationCriticalError(estimation, op) {
        ...
        if (!this.accountState.isSmarterEoa && op.calls.length === 1) {
            ...
            if (!estimateGasError || !ambireError || !bundlerError)
                return null;
            if (ambireError)
    ...
    getBroadcastOption(feeOption, options) {
        ...
        if (isNative(feeToken)) {
            ...
            if (this.accountState.isSmarterEoa)
                return BROADCAST_OPTIONS.bySelf7702;
    ...
    getBroadcastCalldata(accountOp) {
        ...
        return ambireAccount.encodeFunctionData('executeBySender', [getSignableCalls(accountOp)]);
    }
