...
function classifyEmailVaultError(err) {
    ...
    if (msg.includes('invalid key'))
        return 'INVALID_KEY';
    if (msg.includes('email vault not found'))
        return 'NOT_FOUND';
    if (msg.includes('error while creating vault'))
        return 'CREATE_FAILED';
    if (msg.includes('missing params'))
        return 'MISSING_PARAMS';
    return 'UNKNOWN';
}
function friendlyEmailVaultMessage(code, email) {
    switch (code) {
        case 'INVALID_KEY':
