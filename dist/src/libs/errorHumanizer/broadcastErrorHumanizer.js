...
export const PAYMASTER_DOWN_BROADCAST_ERROR_MESSAGE = 'Currently, the paymaster seems to be down and your transaction cannot be broadcast. Please try again in a few moments or pay the fee with a Basic Account if the error persists';
function getPrefix(reason) {
    if (!reason)
        return MESSAGE_PREFIX;
    return !reason.includes('pimlico: 500') ? MESSAGE_PREFIX : '';
}
export function getHumanReadableBroadcastError(e) {
    ...
    let errorMessage = getHumanReadableErrorMessage(commonError, BROADCAST_ERRORS, MESSAGE_PREFIX, decodedError.reason, e);
    if (!errorMessage) {
