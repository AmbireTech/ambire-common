import { allBundlers, BICONOMY, CANDIDE, CUSTOM, ETHERSPOT, GELATO, PIMLICO } from '../../consts/bundlers';
...
export function getDefaultBundlerName(network, opts = { canDelegate: false }) {
 ...
 */
export function getDefaultBundler(network, opts = { canDelegate: false }) {
    return getBundlerByName(getDefaultBundlerName(network, opts));
}
export function getAvailableBundlerNames(network) {
    if (network.customBundlerUrl && network.customBundlerUrl.trim())
        return [CUSTOM];
 ...
 */
export function getAvailableBunlders(network) {
