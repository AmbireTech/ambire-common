    ...
    return (includedSteps
        ...
        gasFees: {
            ...
            asset: step.estimate.gasCosts?.[0]?.token
                ? normalizeLiFiTokenToSwapAndBridgeToToken(step.estimate.gasCosts[0].token, step.estimate.gasCosts[0].token.chainId)
                : undefined
        },
...
const normalizeLiFiRouteToSwapAndBridgeRoute = (route, userAddress) => ({
    routeId: route.id,
...
const normalizeLiFiStepToSwapAndBridgeSendTxRequest = (parentStep) => {
    ...
    return {
        ...
        approvalData: parentStep.action.fromToken.address === constants_1.ZERO_ADDRESS
...
class LiFiAPI {
    ...
    async quote({ fromAsset, fromChainId, fromTokenAddress, toAsset, toChainId, toTokenAddress, fromAmount, userAddress, sort, 
    ...
    isOG }) {
        ...
        const body = {
            ...
            options: {
                ...
                fee: (constants_1.FEE_PERCENT / 100).toString()
            }
        };
        ...
        const routes = response.routes
            .map((r) => normalizeLiFiRouteToSwapAndBridgeRoute(r, userAddress))
            .filter((r) => {
            return !r.usedBridgeNames || r.usedBridgeNames.indexOf(consts_1.MAYAN_BRIDGE) === -1;
        });
        const selectedRoute = response.routes[0]
            ? normalizeLiFiRouteToSwapAndBridgeRoute(response.routes[0], userAddress)
            : undefined;
        const selectedRouteSteps = response.routes[0]
    ...
    async getNextRouteUserTx({ route }) {
