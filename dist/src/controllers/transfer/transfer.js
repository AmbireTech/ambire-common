...
class TransferController extends eventEmitter_1.default {
    ...
    constructor(callRelayer, storage, humanizerInfo, selectedAccount, networks, addressBook, accounts, keystore, portfolio, activity, externalSignerControllers, providers, phishing, dapps, relayerUrl, onBroadcastSuccess, ui, eventEmitterRegistry) {
        ...
        this.#ui.uiEvent.on('removeView', (view) => {
            if (!isTransfer(view.currentRoute) || this.#getHasAnotherTransferViewOpen())
                return;
        ...
        this.#selectedAccount.onUpdate(async (forceEmit) => {
            ...
            if (!this.#currentTransferSessionId || this.hasProceeded)
                return;
            this.#setTokens();
        ...
        const tokenParams = searchParams && searchParams.address && searchParams.chainId
            ? {
    ...
    #setTokens() {
        ...
        if (this.selectedToken) {
            this.selectedToken =
                this.#tokens.find((t) => t.address === this.selectedToken?.address &&
    ...
    #setDefaultSelectedToken(tokenData) {
    ...
    #fetchRecipientAccountStateIfNeeded() {
        ...
        if (recipientAcc && this.selectedToken?.chainId) {
            const state = this.#accounts.accountStates[recipientAcc.addr]?.[this.selectedToken.chainId.toString()];
            if (!state) {
                this.#accounts
    ...
    checkIsRecipientAddressUnknown() {
        ...
        const isAddressInAddressBook = this.#addressBook.contacts.some(({ address }) => address.toLowerCase() === this.recipientAddress.toLowerCase());
    ...
    #setAmountAndNotifyUI(amount) {
        this.amount = amount;
        this.programmaticUpdateCounter += 1;
    }
    #setAmountInFiatAndNotifyUI(amountInFiat) {
        this.amountInFiat = amountInFiat;
        this.programmaticUpdateCounter += 1;
    }
    #setAmount(fieldValue, isProgrammaticUpdate = false) {
    ...
    #setTokenAmount(amount, isProgrammaticUpdate = false) {
    ...
    #getMaxFeeReservationKey() {
        ...
        if (!gasFeePayment || !selectedFeeOption || !selectedToken)
            return null;
        return [
     ...
     */
    #getMaxReservedFeeAmount(feeAmount) {
        const key = this.#getMaxFeeReservationKey();
        if (!key)
            return feeAmount;
        if (!this.#maxFeeReservation ||
    ...
    #shouldReserveFeeFromTransferredToken() {
        ...
        if (!accountAddr || !gasFeePayment || !selectedFeeOption || !selectedToken)
            return false;
        if (selectedFeeOption.token.flags.onGasTank)
            return false;
        if (selectedFeeOption.paidBy.toLowerCase() !== accountAddr)
            return false;
    ...
    #syncAmountWithFeeReservation(forceEmit) {
        ...
        const reservedFee = shouldReserveFee && this.#isMaxAmountSelected ? this.#getMaxReservedFeeAmount(fee) : fee;
        if (!shouldReserveFee)
            this.#resetMaxFeeReservation();
    ...
    async #initSignAccOp(calls, topUpAmount) {
        if (!this.#selectedAccount.account || this.signAccountOpController)
            return;
        const network = this.#networks.networks.find((net) => net.chainId === this.#selectedToken.chainId);
        // shouldn't happen ever
        if (!network)
            return;
        ...
        if (!accountState) {
            const error = new Error(`Failed to fetch account onchain state for network with chainId ${network.chainId}`);
            this.emitError({
                level: 'major',
                message: 'Unable to proceed with the transfer due to missing information (account state). Please try again later.',
                error
            });
            return;
        }
    ...
    destroyLatestBroadcastedAccountOp(skipUpdate = false) {
    ...
    unloadScreen(viewType, opts) {
        ...
        if (this.hasPersistedState && !isNavigateOut && viewType === 'popup')
            return;
        this.reset({ destroyAccountOp: true });
    }
    reset(opts) {
