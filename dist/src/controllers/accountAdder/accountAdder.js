...
class AccountAdderController extends eventEmitter_1.default {
    ...
    hdPathTemplate;
    isInitialized = false;
    isInitializedWithSavedSeed = false;
    ...
    #alreadyImportedAccountsOnControllerInit = [];
    ...
    async #isKeyIteratorInitializedWithTheSavedSeed() {
        if (this.#keyIterator?.subType !== 'seed')
            return false;
        if (!this.#keystore.hasKeystoreSavedSeed)
            return false;
    ...
    async #getInitialHdPathTemplate(defaultHdPathTemplate) {
        if (!this.isInitializedWithSavedSeed)
            return defaultHdPathTemplate;
        const savedSeed = await this.#keystore.getSavedSeed();
        return savedSeed.hdPathTemplate || defaultHdPathTemplate;
    }
    async init({ keyIterator, page, pageSize, hdPathTemplate, shouldSearchForLinkedAccounts = DEFAULT_SHOULD_SEARCH_FOR_LINKED_ACCOUNTS, shouldGetAccountsUsedOnNetworks = DEFAULT_SHOULD_GET_ACCOUNTS_USED_ON_NETWORKS }) {
    ...
    selectAccount(_account) {
        ...
        const accountExists = this.selectedAccounts.some((x) => x.account.addr === nextSelectedAccount.account.addr);
        if (accountExists) {
            ...
            this.selectedAccounts = this.selectedAccounts.map((x) => x.account.addr === nextSelectedAccount.account.addr ? nextSelectedAccount : x);
        }
        else {
    ...
    retrieveInternalKeysOfSelectedAccounts() {
        ...
        return this.#keyIterator?.retrieveInternalKeys(this.selectedAccounts, this.hdPathTemplate, this.#keystore.keys);
    }
    /**
     ...
     */
    async addAccounts(accounts = [], readyToAddKeys = { internal: [], external: [] }) {
        if (!this.isInitialized)
            return this.#throwNotInitialized();
        if (!this.#keyIterator)
            return this.#throwMissingKeyIterator();
        if (!accounts.length) {
            return this.emitError({
                level: 'minor',
                message: 'Trying to add accounts, but no accounts are selected. Please select at least one account.',
                error: new Error('accountAdder: requested method `addAccounts`, but the accounts param is empty')
            });
        }
    ...
    async #getAccountsUsedOnNetworks({ accounts }) {
        ...
        const sortedAccountsWithNetworksArray = finalAccountsWithNetworksArray.sort((a, b) => {
            ...
            const networkIndexA = this.#networks.networks.findIndex((network) => networkIdsA.includes(network.id));
            const networkIndexB = this.#networks.networks.findIndex((network) => networkIdsB.includes(network.id));
    ...
    #throwNotInitialized() {
        this.emitError({
            ...
            error: new Error('accountAdder: requested a method of the AccountAdder controller, but the controller was not initialized')
        });
    }
