...
class ContinuousUpdatesController extends eventEmitter_1.default {
    ...
    constructor({ eventEmitterRegistry, main }) {
        ...
        this.#main.requests.onUpdate(() => {
            if (this.#main.requests.currentUserRequest?.kind === 'calls') {
                if (!this.#main.requests.currentUserRequest.signAccountOp.onUpdateIds.includes('continuous-update')) {
        ...
        this.#main.activity.onUpdate(() => {
            const allBroadcastedButNotConfirmed = Object.values(this.#main.activity.broadcastedButNotConfirmed).flat();
    ...
    async #updateAccountStateLatest() {
        ...
        if (!this.#main.accounts.accounts.length)
            return; // no accounts imported yet
        ...
        const networksToUpdate = this.#main.networks.networks
            .filter(({ chainId }) => !networksWithPendingAccountOp.includes(chainId) &&
            !failedChainIds.includes(chainId.toString()))
            .map(({ chainId }) => chainId);
        await this.#main.accounts.updateAccountState(this.#main.selectedAccount.account.addr, 'latest', networksToUpdate);
    }
    async #updateAccountStatePending() {
        ...
        await this.#main.accounts.updateAccountState(this.#main.selectedAccount.account.addr, 'pending', networksToUpdate);
    }
    async #fastAccountStateReFetch() {
        ...
        await this.#main.accounts.updateAccountState(selectedAccountAddr, 'latest', chainIdsToRetry.map((id) => BigInt(id)));
        ...
        const networksNotYetRetried = failedChainIdsAfterUpdate.filter((id) => !this.#accountStateRetriesByNetwork[id]);
    ...
    async #resolveConfirmedSafeTxns() {
        ...
        if (!this.#main.keystore.isUnlocked)
            return;
        const pendingSafeTxns = this.#main.requests.userRequests
            .filter((r) => r.meta.accountAddr === this.#main.selectedAccount.account?.addr &&
            r.kind === 'calls' &&
            !!r.signAccountOp.account.safeCreation &&
            r.signAccountOp.accountOp.txnId &&
            r.signAccountOp.accountOp.signed?.length)
            .map((r) => {
            const accountOp = r.signAccountOp.accountOp;
            return {
                chainId: accountOp.chainId,
                safeTxnHash: accountOp.txnId
            };
        });
        if (!pendingSafeTxns.length)
            return;
        ...
        for (let i = 0; i < confirmed.length; i++) {
            ...
            const userR = this.#main.requests.userRequests.find((r) => r.kind === 'calls' &&
                !!r.signAccountOp.account.safeCreation &&
                oneConfirmed.safeTxnHash === r.signAccountOp.accountOp.txnId);
            if (!userR)
                continue;
            const callsUserR = userR;
            if (oneConfirmed.transactionHash) {
                ...
                if (commonSuccessHandler instanceof Error)
                    continue;
    ...
    async #resolveConfirmedSafeMessages() {
        ...
        if (!this.#main.selectedAccount.account || !this.#main.selectedAccount.account.safeCreation)
            return;
        const accountStates = await this.#main.accounts.getOrFetchAccountStates(this.#main.selectedAccount.account.addr);
        if (!accountStates)
            return;
        ...
        if (!pendingSafeMessages.length)
            return;
        const msgs = (await this.#main.safe.getMessagesByHash(pendingSafeMessages)).filter((m) => m.isConfirmed);
        // resolve each request
        for (let i = 0; i < msgs.length; i++) {
            const msg = msgs[i];
            const userR = this.#main.requests.userRequests.find((r) => (r.kind === 'message' || r.kind === 'typedMessage' || r.kind === 'siwe') &&
