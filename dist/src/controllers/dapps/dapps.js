...
export class DappsController extends EventEmitter {
    ...
    #recentDapps = [];
    dappToConnect = null;
    isReadyToDisplayDapps = true;
    fetchAndUpdatePromise;
    #shouldRetryFetchAndUpdate = false;
    #retryFetchAndUpdateInterval;
    #retryFetchAndUpdateAttempts = 0;
    #retryFetchAndUpdateMaxAttempts = 3;
    ...
    constructor({ eventEmitterRegistry, appVersion, fetch, storage, networks, phishing, ui }) {
        ...
        this.#phishing.onUpdate(() => {
            if (!this.#phishing.shouldSyncDapps)
                return;
        ...
        this.#ui.uiEvent.on('removeView', (removedView) => {
            if (removedView.type === 'popup' &&
    ...
    async #fetchAndUpdateDapps() {
        ...
        if (lastDappsUpdateVersion && lastDappsUpdateVersion === this.#appVersion) {
            const dappsWithoutBlacklistedStatus = Array.from(this.#dapps.values()).filter((d) => !!d.blacklisted && ['LOADING', 'FAILED_TO_GET'].includes(d.blacklisted));
        ...
        if (this.#shouldRetryFetchAndUpdate)
            this.#retryFetchAndUpdateAttempts += 1;
        ...
        for (const dapp of fetchedDappsList) {
            ...
            const chainIds = (dapp.chains ?? [])
                .map((chainName) => chainNamesToIds.get(chainName.toLowerCase()))
    ...
    async #updateDappsBlacklistedStatus(dapps) {
        await this.#phishing.updateDomainsBlacklistedStatus(dapps.map((d) => d.url), (blacklistedStatus) => {
    ...
    #syncDappsBlacklistedStatusWithPhishing() {
        ...
        this.#dapps.forEach((dapp, dappId) => {
            ...
            if (!updatedStatus || dapp.blacklisted === updatedStatus)
                return;
    ...
    getDappSessionByWcTopic(wcTopic) {
        return Object.values(this.dappSessions).find((session) => session.wcTopic === wcTopic);
    }
    setSessionMessenger = (sessionId, messenger, isAmbireNext) => {
        this.dappSessions[sessionId]?.setMessenger(messenger, isAmbireNext);
    };
    setSessionLastHandledRequestsId = (sessionId, providerId, id, isWeb3AppRequest) => {
        if (!this.dappSessions[sessionId])
            return;
    ...
    resetSessionLastHandledRequestsId = (sessionId, providerId) => {
        if (providerId) {
    ...
    setSessionProp = (sessionId, props) => {
        this.dappSessions[sessionId]?.setProp(props);
    };
    deleteDappSession = (sessionId) => {
        delete this.dappSessions[sessionId];
        this.emitUpdate();
    };
    deleteDappSessionByWcTopic = (wcTopic) => {
    ...
    broadcastDappSessionEvent = async (ev, data, id, skipPermissionCheck, sourceFilter) => {
        await this.initialLoadPromise;
        let dappSessions = [];
        Object.keys(this.dappSessions).forEach((sessionId) => {
            const hasPermissionToBroadcast = skipPermissionCheck || this.hasPermission(this.dappSessions[sessionId].id);
        ...
        if (sourceFilter) {
            dappSessions = dappSessions.filter((dappSession) => sourceFilter === 'wc' ? !!dappSession.data.wcTopic : !dappSession.data.wcTopic);
        }
    ...
    async #buildDapp(dapp) {
        ...
        const network = this.#networks.allNetworks.find((n) => n.chainId.toString() === dapp.chainId?.toString());
     ...
     */
    pickWalletConnectChainId(candidateChainIds) {
        if (!candidateChainIds?.length)
            return undefined;
     ...
     */
    async addDappFromIdentity(identity, source) {
        if (!this.isReady)
            return;
        await this.initialLoadPromise;
     ...
     */
    async addDapp(dapp, source = 'injected') {
        if (!this.isReady)
            return;
        const existing = this.#dapps.get(dapp.id);
        if (existing) {
        ...
        const sources = dapp.isConnected ? [source] : [];
    ...
    updateDapp(id, dapp) {
        ...
        if (dappPropsToUpdate.connectedSources !== undefined) {
            ...
            const nextSources = dappPropsToUpdate.isConnected
     ...
     */
    async disconnectDappSource(id, source) {
        if (!this.isReady)
            return;
        const existing = this.#dapps.get(id);
        if (!existing)
    ...
    async clearRecentDapps() {
        await this.initialLoadPromise;
        if (!this.#recentDapps.length)
            return;
        this.#recentDapps = [];
        await this.#storage.set('recentDapps', this.#recentDapps);
        this.emitUpdate();
    }
    hasPermission(id, source) {
        if (!id)
            return false;
    ...
    async getCurrentDappAndSendResToUi({ requestId, dappId, currentSessionId = '' }) {
    ...
    hasUnverifiedDappUrls(dapps) {
        const verifiedDappUrlsSet = new Set();
    ...
    async hasUnverifiedDappsAndSendResToUi({ requestId, dapps }) {
     ...
     */
    getDappVerificationBanner(dappUrls, { includeDappNamesInText = true } = {}) {
        ...
        const getDappNamesByPredicate = (predicate) => Array.from(new Set(dappVerificationData.filter(predicate).map((dapp) => dapp.name))).join(', ');
        // Conditionally appends the matching dApp names, separated by ":" for readability.
        const withOptionalDappNames = (baseText, dappNames) => {
            if (!includeDappNamesInText || !dappNames)
                return baseText;
        ...
        const isDappInDefaultCatalog = (dappId) => {
