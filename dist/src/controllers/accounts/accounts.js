...
export class AccountsController extends EventEmitter {
    ...
    accountStateInitialLoadPromise;
    constructor(storage, providers, networks, keystore, onAddAccounts, updateProviderIsWorking, onAccountStateUpdate, relayerUrl, fetch, eventEmitterRegistry) {
    ...
    #getAccountsToUpdateAccountStatesInBackground(selectedAccountAddr) {
    ...
    async #load() {
        ...
        // --------------------------------------------------
        this.accountStateInitialLoadPromise = this.updateAccountStates(this.#getAccountsToUpdateAccountStatesInBackground(initialSelectedAccountAddr)).finally(() => {
    ...
    set accounts(nextAccounts) {
        ...
        if (noNewAccountsAndNotInitialLoad)
            return;
            ...
            try {
                ...
                networkAccountStates.forEach((accountState) => {
                    ...
                    if (blockTag === 'pending') {
                        ...
                        const toState = current && current.currentBlock >= accountState.currentBlock
                            ? current
                            : accountState;
    ...
    async #addAccounts(accounts = []) {
        ...
        const nextAccounts = [
            ...this.accounts.map((acc) => ({
                ...
                safeCreation: acc.safeCreation ?? newAccountsAlreadyAdded.find((x) => x.addr === acc.addr)?.safeCreation
            })),
    ...
    async reorderAccounts({ fromIndex, toIndex }) {
        if (fromIndex < 0 ||
            toIndex < 0 ||
    ...
    async setViewOnlyAccountIdentitiesIfNeeded() {
        const viewOnlyAccountsNeedingIdentityFetch = this.accounts.filter((a) => !this.#keystore.getAccountKeys(a).length && !a.identityFetchedAt && !a.safeCreation);
        if (!viewOnlyAccountsNeedingIdentityFetch.length)
            return this.#viewOnlyAccountGetIdentityInterval.stop();
        const accountsToFetchIdentity = viewOnlyAccountsNeedingIdentityFetch.map(async (a) => {
            const identityRes = await this.#callRelayer(`/v2/identity/${a.addr}`).catch((err) => {
                // 404 response (if the account is not found) is a valid response, do not throw
                if (err?.output?.res?.status === 404)
                    return null;
                throw err;
            });
        ...
        const fulfilledAccountsWithIdentities = results
    ...
    async createSmartAccountIdentitiesIfNeeded() {
        ...
        const identityRequests = smartAccountsNeedingIdentityCreate.map((account) => ({
            addr: account.addr,
        ...
        try {
            ...
            this.accounts = this.accounts.map((account) => {
                if (!identityExists.includes(account.addr))
                    return account;
                // should never happen
                if (!account.creation) {
                ...
                // privileges, because key management for them got never implemented.
                const initialPrivileges = identityRequests.find((a) => a.addr === account.addr)?.associatedKeys ||
                    account.initialPrivileges;
