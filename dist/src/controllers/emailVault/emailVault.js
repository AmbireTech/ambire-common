...
export class EmailVaultController extends EventEmitter {
    ...
    #isRemovingSecret = false;
    #emailVault;
    #magicLinkKeys = {};
    #sessionKeys = {};
    #shouldStopConfirmationPolling = false;
    #autoConfirmMagicLink = false;
    #fetch;
    #relayerUrl;
    #keyStore;
    isReady = false;
    ...
    constructor(storage, fetch, relayerUrl, keyStore, options, eventEmitterRegistry) {
    ...
    get currentState() {
        ...
        if (this.#isWaitingEmailConfirmation)
            return EmailVaultState.WaitingEmailConfirmation;
        if (this.#isUploadingSecret)
            return EmailVaultState.UploadingSecret;
        if (this.#isRemovingSecret)
            return EmailVaultState.RemovingSecret;
        return EmailVaultState.Ready;
    }
    async #requestSessionKey(email) {
        ...
        polling.onUpdate(async (forceEmit) => {
            if (polling.state.isError && polling.state.error.output.res.status === 401) {
                ...
                this.emitError({
                    ...
                    level: 'major',
                    error: new Error(`Can't request magic link for email ${email}: ${polling.state.error.message}`)
                });
                this.emailVaultStates.errors = [polling.state.error];
                this.propagateUpdate(forceEmit);
            }
        });
        const ev = await polling.exec(this.#emailVault.getEmailVaultInfo.bind(this.#emailVault), [email, newKey.key], () => {
            this.#isWaitingEmailConfirmation = false;
        }, () => this.#shouldStopConfirmationPolling, 3 * 60 * 1000, 1000);
    ...
    getMagicLinkKeyByEmail(email) {
        ...
        if (!result || !result.confirmed)
            return null;
        if (Date.now() >= result.expiry.getTime())
            return null;
        return result;
    }
    async #getMagicLinkKey(email) {
        // if we have valid magicLinkKey => returns it else null
        await this.#initialLoadPromise;
        return this.getMagicLinkKeyByEmail(email);
    }
    #parseMagicLinkKeys(mks) {
        return Object.fromEntries(Object.keys(mks).map((email) => [
            email,
    ...
    async #getEmailVaultInfo(email, flow) {
        ...
        let emailVault = null;
        if (key) {
            emailVault = await this.#emailVault.getEmailVaultInfo(email, key).catch((e) => {
                this.emitError({
    ...
    async removeKeyStoreSecret(email) {
    ...
    async #removeKeyStoreSecret(email) {
        ...
        try {
            ...
            const keyStoreUid = await this.#keyStore.getKeyStoreUid();
            result = await this.#emailVault.removeKeyStoreSecretFromRelayer(email, magicKey.key, keyStoreUid);
            if (result) {
    ...
    async #recoverKeyStore(email, newPassword) {
        ...
        if (!result || !result.value) {
            this.emitError({
                message: 'Something goes wrong while we are resetting your password! Please try again! If the problem persists, please contact support',
                level: 'major',
                error: new Error("Keystore recovery: retrieveKeyStoreSecret doesn't return result or result.value.")
            });
            return;
        }
    ...
    async #finalizeSyncKeys(email, operations) {
                ...
                if (op.type === 'requestKeySync') {
                    return {
                        ...
                        value: JSON.stringify({
                            privateKey: await this.#keyStore.exportKeyWithPublicKeyEncryption(op.key, op.requester)
                        }),
                        password
                    };
                }
