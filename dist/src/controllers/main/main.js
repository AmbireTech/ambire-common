...
const helper_1 = require("../signAccountOp/helper");
const signAccountOp_2 = require("../signAccountOp/signAccountOp");
...
class MainController extends eventEmitter_1.default {
    ...
    constructor({ platform, storageAPI, fetch, relayerUrl, velcroUrl, swapApiKey, keystoreSigners, externalSignerControllers, windowManager, notificationManager }) {
        ...
        this.storage = new storage_1.StorageController(this.#storageAPI);
        this.invite = new invite_1.InviteController({ relayerUrl, fetch, storage: this.storage });
        this.keystore = new keystore_1.KeystoreController(platform, this.storage, keystoreSigners, windowManager);
        this.#externalSignerControllers = externalSignerControllers;
        this.networks = new networks_2.NetworksController(this.storage, this.fetch, relayerUrl, async (network) => {
            if (network.disabled) {
        ...
        this.featureFlags = new featureFlags_1.FeatureFlagsController(this.networks);
        this.providers = new providers_1.ProvidersController(this.networks);
        this.accounts = new accounts_1.AccountsController(this.storage, this.providers, this.networks, async (accounts) => {
        ...
        this.portfolio = new portfolio_1.PortfolioController(this.storage, this.fetch, this.providers, this.networks, this.accounts, this.keystore, relayerUrl, velcroUrl);
        ...
        this.signMessage = new signMessage_1.SignMessageController(this.keystore, this.providers, this.networks, this.accounts, this.#externalSignerControllers, this.invite, () => {
        ...
        const lifiAPI = new api_1.LiFiAPI({ apiKey: swapApiKey, fetch: this.fetch });
        ...
        this.swapAndBridge = new swapAndBridge_2.SwapAndBridgeController({
            ...
            isMainSignAccountOpThrowingAnEstimationError: (fromChainId, toChainId) => {
                return (this.signAccountOp &&
                    fromChainId &&
                    toChainId &&
                    this.signAccountOp.estimation.status === types_2.EstimationStatus.Error &&
    ...
    async #selectAccount(toAccountAddr) {
        ...
        const swapAndBridgeSigningAction = this.actions.visibleActionsQueue.find(({ type }) => type === 'swapAndBridge');
    ...
    initSignAccOp(actionId) {
        ...
        if (!this.signAccountOp || this.signAccountOp.fromActionId !== actionId) {
            ...
            this.signAccountOp = new signAccountOp_2.SignAccountOpController(this.accounts, this.networks, this.keystore, this.portfolio, this.#externalSignerControllers, this.selectedAccount.account, network, this.providers.providers[network.chainId.toString()], actionId, accountOp, () => {
    ...
    async handleSignAndBroadcastAccountOp(type) {
        if (this.statuses.signAndBroadcastAccountOp !== 'INITIAL') {
            this.emitError({
                level: 'major',
                message: 'The signing process is already in progress.',
                error: new Error('The signing process is already in progress. (handleSignAndBroadcastAccountOp)')
            });
            return;
        }
        const signAndBroadcastCallId = (0, uuid_1.generateUuid)();
        this.#signAndBroadcastCallId = signAndBroadcastCallId;
        this.statuses.signAndBroadcastAccountOp = 'SIGNING';
        this.forceEmitUpdate();
        const signAccountOp = type === helper_1.SIGN_ACCOUNT_OP_MAIN
            ? this.signAccountOp
        ...
        try {
            ...
            const wasAlreadySigned = signAccountOp?.status?.type === signAccountOp_2.SigningStatus.Done;
            if (!wasAlreadySigned) {
                if (!signAccountOp) {
                    ...
                    throw new EmittableError_1.default({ level: 'major', message });
                }
            ...
            if (signAccountOp?.status?.type !== signAccountOp_2.SigningStatus.Done) {
            ...
            if (signAndBroadcastCallId === this.#signAndBroadcastCallId) {
                if ('message' in error && 'level' in error && 'error' in error) {
                    ...
                    const hasSigned = signAccountOp?.status?.type === signAccountOp_2.SigningStatus.Done;
                    this.emitError({
                        level: 'major',
    ...
    #abortHWSign(signAccountOp) {
        if (!signAccountOp)
            return;
        const isAwaitingHWSignature = (signAccountOp.accountOp.signingKeyType !== 'internal' &&
            this.statuses.signAndBroadcastAccountOp === 'SIGNING') ||
        ...
        const isSignerTrezor = signAccountOp.accountOp.signingKeyType === 'trezor' || this.feePayerKey?.type === 'trezor';
        if (isSignerTrezor) {
            this.#handleTrezorCleanup();
        }
        this.#signAccountOpSigningPromise = undefined;
    ...
    async traceCall(signAccountOpCtrl) {
        ...
        if (signAccountOpCtrl.traceCallDiscoveryStatus === signAccountOp_1.TraceCallDiscoveryStatus.NotStarted)
            signAccountOpCtrl.setDiscoveryStatus(signAccountOp_1.TraceCallDiscoveryStatus.InProgress);
        // Flag the discovery logic as `SlowPendingResponse` if the call does not resolve within 2 seconds.
        const timeoutId = setTimeout(() => {
            signAccountOpCtrl.setDiscoveryStatus(signAccountOp_1.TraceCallDiscoveryStatus.SlowPendingResponse);
        ...
        try {
            ...
            const stateOverride = accountOp.calls.length > 1 && (0, account_1.isBasicAccount)(account, state)
                ? {
                    [account.addr]: {
                ...
                : undefined;
            const { tokens, nfts } = await (0, debugTraceCall_1.debugTraceCall)(account, accountOp, provider, state, !network.rpcNoStateOverride, stateOverride);
            ...
            const accountOpsForSimulation = (0, main_1.getAccountOpsForSimulation)(account, this.actions.visibleActionsQueue, this.networks.networks);
            ...
            if (canUpdateSignAccountOp && (learnedNewTokens || learnedNewNfts)) {
                await this.portfolio.updateSelectedAccount(accountOp.accountAddr, network, accountOpsForSimulation
                    ? {
                        accountOps: accountOpsForSimulation,
                        states: await this.accounts.getOrFetchAccountStates(account.addr)
                    }
                    : undefined, { forceUpdate: true });
            }
            signAccountOpCtrl.setDiscoveryStatus(signAccountOp_1.TraceCallDiscoveryStatus.Done);
        }
        catch (e) {
            signAccountOpCtrl.setDiscoveryStatus(signAccountOp_1.TraceCallDiscoveryStatus.Failed);
            this.emitError({
                level: 'silent',
    ...
    async #ensureAccountInfo(accountAddr, chainId) {
        await this.#initialLoadPromise;
        ...
        if (!this.accounts.accountStates[accountAddr]?.[chainId.toString()]) {
            ...
            return {
                hasAccountInfo: false,
                errorMessage: `We couldn't complete your last action because we couldn't retrieve your account information for ${network?.name || chainId}. Please try reloading your account from the Dashboard. If the issue persists, contact support for assistance.`
            };
        }
        return {
            hasAccountInfo: true
    ...
    #batchCallsFromUserRequests(accountAddr, chainId) {
    ...
    async reloadSelectedAccount(options) {
        ...
        await Promise.all([
            ...
            !this.accounts.areAccountStatesLoading && this.selectedAccount.account?.addr
                ? this.accounts.updateAccountState(this.selectedAccount.account.addr, 'pending', chainId ? [chainId] : undefined)
                : Promise.resolve(),
    ...
    #updateIsOffline() {
        ...
        const isAllLoaded = latestStateKeys.every((chainId) => {
            return (0, selectedAccount_1.isNetworkReady)(latestState[chainId]) && !latestState[chainId]?.isLoading;
        });
    ...
    forceUpdate = false, network, maxDataAgeMs) {
        ...
        if (!canUpdateSignAccountOp)
            return;
        const accountOpsToBeSimulatedByNetwork = (0, main_1.getAccountOpsForSimulation)(this.selectedAccount.account, this.actions.visibleActionsQueue, this.networks.networks);
        await this.portfolio.updateSelectedAccount(this.selectedAccount.account.addr, network, accountOpsToBeSimulatedByNetwork
            ? {
            ...
            : undefined, { forceUpdate, maxDataAgeMs });
     ...
     */
    async #swapAndBridgeActionSafeguard(throwRpcError = false) {
        const pendingSwapAction = this.actions.visibleActionsQueue.find(({ type }) => type === 'swapAndBridge');
        if (!pendingSwapAction)
            return false;
        const isSigningOrBroadcasting = this.statuses.signAndBroadcastAccountOp === 'SIGNING' ||
            this.statuses.signAndBroadcastAccountOp === 'BROADCASTING';
        ...
        if (throwRpcError) {
            throw eth_rpc_errors_1.ethErrors.rpc.transactionRejected({
    ...
    async buildUserRequestFromDAppRequest(request, dappPromise) {
        ...
        if (kind === 'calls') {
            ...
            const baseAcc = (0, getBaseAccount_1.getBaseAccount)(this.selectedAccount.account, await this.accounts.getOrFetchAccountOnChainState(this.selectedAccount.account.addr, network.chainId), this.keystore.getAccountKeys(this.selectedAccount.account), network);
            ...
            const paymasterService = isWalletSendCalls && !!request.params[0].capabilities?.paymasterService
                ...
                : (0, erc7677_1.getAmbirePaymasterService)(baseAcc, this.#relayerUrl);
            ...
            const accountState = await this.accounts.getOrFetchAccountOnChainState(accountAddr, network.chainId);
            if ((0, account_1.isBasicAccount)(this.selectedAccount.account, accountState)) {
    ...
    async buildTransferUserRequest(amount, recipientAddress, selectedToken, actionExecutionType = 'open-action-window') {
        ...
        if (!this.selectedAccount.account)
            return;
        const baseAcc = (0, getBaseAccount_1.getBaseAccount)(this.selectedAccount.account, await this.accounts.getOrFetchAccountOnChainState(this.selectedAccount.account.addr, selectedToken.chainId), this.keystore.getAccountKeys(this.selectedAccount.account), this.networks.networks.find((net) => net.chainId === selectedToken.chainId));
    ...
    async buildSwapAndBridgeUserRequest(activeRouteId) {
        await this.withStatus('buildSwapAndBridgeUserRequest', async () => {
            ...
            if (this.swapAndBridge.toSelectedToken && this.swapAndBridge.toChainId) {
                this.portfolio.addTokensToBeLearned([this.swapAndBridge.toSelectedToken.address], BigInt(this.swapAndBridge.toChainId));
            }
            ...
            if (activeRoute) {
                ...
                if (transaction) {
                    ...
                    if ((0, account_1.isBasicAccount)(this.selectedAccount.account, await this.accounts.getOrFetchAccountOnChainState(this.selectedAccount.account.addr, network.chainId))) {
            ...
            const baseAcc = (0, getBaseAccount_1.getBaseAccount)(this.selectedAccount.account, accountState, this.keystore.getAccountKeys(this.selectedAccount.account), network);
            const swapAndBridgeUserRequests = await (0, swapAndBridge_1.buildSwapAndBridgeUserRequests)(transaction, network.chainId, this.selectedAccount.account, this.providers.providers[network.chainId.toString()], accountState, (0, erc7677_1.getAmbirePaymasterService)(baseAcc, this.#relayerUrl));
    ...
    rejectUserRequest(err, requestId) {
        ...
        if (userRequest.action.kind === 'calls') {
            ...
            if ((0, account_1.isBasicAccount)(acc, this.accounts.accountStates[acc.addr][userRequest.meta.chainId]) &&
        ...
        if (requestId) {
            ...
            if (userRequest.action.kind === 'calls') {
                ...
                if (userRequest.action.calls.length === 0) {
                    ...
                    const accountOpAction = (0, main_1.makeAccountOpAction)({
    ...
    async addUserRequest(req, actionPosition = 'last', actionExecutionType = 'open-action-window', allowAccountSwitch = false) {
        ...
        if (action.kind === 'calls') {
            ...
            if (!accountInfo.hasAccountInfo) {
                ...
                req.dappPromise?.reject(eth_rpc_errors_1.ethErrors.provider.custom({
                    code: 1001,
                ...
                throw new EmittableError_1.default({
                    level: 'major',
                    message: accountInfo.errorMessage,
                    error: new Error(`Couldn't retrieve account information for network with id ${meta.chainId}, because of one of the following reasons: 1) network doesn't exist, 2) RPC is down for this network.`)
                });
            }
            ...
            const accountState = await this.accounts.getOrFetchAccountOnChainState(meta.accountAddr, meta.chainId);
            ...
            if (req.action.kind === 'authorization-7702')
                actionType = 'signMessage';
    ...
    removeUserRequest(id, options) {
        const { shouldRemoveSwapAndBridgeRoute = true, shouldUpdateAccount = true, shouldOpenNextRequest = true } = options || {};
        ...
        if (action.kind === 'calls') {
            ...
            const accountOpIndex = this.actions.actionsQueue.findIndex((a) => a.type === 'accountOp' && a.id === `${meta.accountAddr}-${meta.chainId}`);
            ...
            accountOpAction.accountOp.calls = this.#batchCallsFromUserRequests(meta.accountAddr, meta.chainId);
            ...
            const requestsToAddOrRemove = this.userRequestWaitingAccountSwitch.filter((r) => r.meta.accountAddr === this.selectedAccount.account.addr);
            const isSelectedAccountSwitched = this.selectedAccount.account?.addr === action.params.switchToAccountAddr;
    ...
    async removeNetworkData(chainId) {
    ...
    async resolveAccountOpAction(submittedAccountOp, actionId, isBasicAccountBroadcastingMultiple) {
        ...
        for (const call of calls) {
            ...
            if (walletSendCallsUserReq) {
                walletSendCallsUserReq.dappPromise?.resolve({
                    hash: (0, submittedAccountOp_1.getDappIdentifier)(submittedAccountOp)
                });
     ...
     *
     */
    async #broadcastSignedAccountOp(signAccountOp, type, callId) {
        ...
        if (rawTxnBroadcast.includes(accountOp.gasFeePayment.broadcastOption)) {
            ...
            const senderAddr = broadcast_1.BROADCAST_OPTIONS.byOtherEOA
            ...
            try {
                ...
                const txnLength = baseAcc.shouldBroadcastCallsSeparately(accountOp)
                    ...
                    : 1;
                if (txnLength > 1)
                    signAccountOp.update({ signedTransactionsCount: 0 });
                for (let i = 0; i < txnLength; i++) {
                    const currentNonce = nonce + i;
                    const rawTxn = await (0, broadcast_1.buildRawTransaction)(account, accountOp, accountState, provider, network, currentNonce, accountOp.gasFeePayment.broadcastOption, accountOp.calls[i]);
                    const signedTxn = accountOp.gasFeePayment.broadcastOption === broadcast_1.BROADCAST_OPTIONS.delegation
                    ...
                    if (accountOp.gasFeePayment.broadcastOption === broadcast_1.BROADCAST_OPTIONS.delegation) {
                    ...
                    if (txnLength > 1)
                        signAccountOp.update({ signedTransactionsCount: i + 1 });
                    // send the txn to the relayer if it's an EOA sending for itself
                    if (accountOp.gasFeePayment.broadcastOption !== broadcast_1.BROADCAST_OPTIONS.byOtherEOA) {
                ...
                transactionRes = {
                    ...
                    txnId: txnLength === 1 ? multipleTxnsBroadcastRes.map((res) => res.hash).join('-') : undefined
                };
            }
            catch (error) {
        ...
        // Smart account, the ERC-4337 way
            ...
            try {
                ...
                if (signAccountOp) {
                    ...
                    signAccountOp.updateStatus(signAccountOp_2.SigningStatus.ReadyToSign);
        ...
        if (type === helper_1.SIGN_ACCOUNT_OP_SWAP) {
        ...
        if (isBasicAccountBroadcastingMultiple) {
            ...
            const calls = submittedAccountOp.calls
                ...
                localCall.status = types_1.AccountOpStatus.BroadcastedButNotConfirmed;
                return localCall;
            })
        ...
        if (type === helper_1.SIGN_ACCOUNT_OP_MAIN) {
            await this.resolveAccountOpAction(submittedAccountOp, actionId, isBasicAccountBroadcastingMultiple);
        }
    ...
    get banners() {
        ...
        return (0, banners_1.getAccountOpBanners)({
    ...
    throwBroadcastAccountOp({ signAccountOp, message: humanReadableMessage, error: _err, accountState, isRelayer = false, provider = undefined, network = undefined }) {
        ...
        signAccountOp?.updateStatus(signAccountOp_2.SigningStatus.ReadyToSign, isReplacementFeeLow);
        this.feePayerKey = null;
        // remove the active route on broadcast failure
        if (signAccountOp?.accountOp.meta?.swapTxn) {
            this.swapAndBridge.removeActiveRoute(signAccountOp.accountOp.meta.swapTxn.activeRouteId);
        }
        throw new EmittableError_1.default({ level: 'major', message, error: _err || new Error(message) });
    }
