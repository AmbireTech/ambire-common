...
class TransactionFormState extends eventEmitter_1.default {
    ...
    async update(params, updateProps) {
        ...
        if (fromSelectedToken) {
            ...
            if (isFromNetworkChanged) {
                const network = this.dependencies.networks.networks.find((n) => n.chainId === fromSelectedToken.chainId);
                if (network) {
    ...
    unloadScreen(sessionId, forceUnload) {
        const isFormDirty = !!this.fromAmount || !!this.toSelectedToken || !!this.addressState.fieldValue;
        ...
        const shouldPersistState = isFormDirty && sessionId === 'popup' /*  || isSigningOrBroadcasting */ && !forceUnload;
    ...
    async updateToTokenList(shouldReset, addressToSelect) {
        ...
        const toTokenNetwork = this.dependencies.networks.networks.find((n) => Number(n.chainId) === this.toChainId);
        // should never happen
        if (!toTokenNetwork) {
            this.updateToTokenListStatus = 'INITIAL';
            this.emitUpdate();
    ...
    async switchFromAndToTokens() {
        ...
        if (!this.toSelectedToken) {
            ...
            const toSelectedTokenNetwork = this.dependencies.networks.networks.find((n) => Number(n.chainId) === this.toChainId);
    ...
    async #load() {
        ...
        this.activeRoutes = this.activeRoutes.filter((r) => r.serviceProviderId === this.dependencies.serviceProviderAPI.id && r.routeStatus !== 'ready');
    ...
    get recipientAddress() {
        return (this.addressState.resolvedAddress ||
