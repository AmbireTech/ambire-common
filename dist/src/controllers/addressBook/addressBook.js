...
export class AddressBookController extends EventEmitter {
    ...
    #accounts;
    #selectedAccount;
    constructor(storage, accounts, selectedAccount, eventEmitterRegistry) {
    ...
    get contacts() {
        if (!this.#selectedAccount.account)
            return [];
        return [...this.#manuallyAddedContacts, ...this.#walletAccountsSourcedContacts].filter(({ address }) => address !== this.#selectedAccount.account.addr);
    }
    async #load() {
        ...
        try {
            ...
            this.emitError({
                message: 'Something went wrong when loading the Address Book. Please try again or contact support if the problem persists.',
                level: 'major',
    ...
    #findManuallyAddedContactWithAddress(address) {
        return this.#manuallyAddedContacts.find((contact) => contact.address.toLowerCase() === address.toLowerCase());
    }
    #findContactWithAddress(address) {
    ...
    async addContact(name, address) {
        ...
        if (!this.#findContactWithAddress(checksummedAddress)) {
            this.emitError({
                ...
                level: 'minor',
                error: new Error('Address Book: contact with this address already exists in the Address Book')
            });
            return;
        }
    ...
    async renameManuallyAddedContact(address, newName) {
        ...
        if (!this.#findManuallyAddedContactWithAddress(checksummedAddress)) {
            this.emitError({
                ...
                level: 'minor',
                error: new Error("Address Book: can't rename contact that doesn't exist in the Address Book")
            });
            return;
        }
    ...
    async removeManuallyAddedContact(address) {
        ...
        if (!this.#findManuallyAddedContactWithAddress(checksummedAddress)) {
            this.emitError({
                ...
                level: 'minor',
                error: new Error("Address Book: can't remove contact that doesn't exist in the Address Book")
            });
            return;
        }
        this.#manuallyAddedContacts = this.#manuallyAddedContacts.filter((contact) => contact.address.toLowerCase() !== address.toLowerCase());
        this.#handleManuallyAddedContactsChange();
    }
    toJSON() {
        return {
