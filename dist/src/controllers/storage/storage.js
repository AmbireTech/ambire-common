...
class StorageController extends eventEmitter_1.default {
    ...
    async #migrateCashbackStatusToNewFormat() {
        ...
        if (passedMigrations.includes('migrateCashbackStatusToNewFormat'))
            return;
        const migratedCashbackStatusByAccount = Object.fromEntries(Object.entries(cashbackStatusByAccount).map(([accountId, status]) => {
            if (typeof status === 'string') {
                return [accountId, status];
            }
            if (typeof status === 'object' && status !== null) {
                const { cashbackWasZeroAt, firstCashbackReceivedAt, firstCashbackSeenAt } = status;
                if (cashbackWasZeroAt &&
                    firstCashbackReceivedAt === null &&
                    firstCashbackSeenAt === null) {
                    return [accountId, 'no-cashback'];
                }
                if (cashbackWasZeroAt === null &&
    ...
    async #migrateTokenPreferences() {
        ...
        if (tokenPreferences.some(({ symbol, decimals }) => !!symbol || !!decimals)) {
            storageUpdates.push(this.#storage.set('tokenPreferences', (0, storage_1.migrateHiddenTokens)(tokenPreferences)));
            storageUpdates.push(this.#storage.set('customTokens', (0, storage_1.migrateCustomTokens)(tokenPreferences)));
        }
        await Promise.all(storageUpdates);
    }
    async #migrateNetworkIdToChainId() {
        ...
        const migratedPreviousHints = {
            ...
            fromExternalAPI: Object.fromEntries(Object.entries(previousHints.fromExternalAPI || {}).map(([networkAndAccountKey, value]) => {
    ...
    async #associateAccountKeysWithLegacySavedSeedMigration(accountPicker, keystore, onSuccess) {
    ...
    async associateAccountKeysWithLegacySavedSeedMigration(accountPicker, keystore, onSuccess) {
        await this.withStatus('associateAccountKeysWithLegacySavedSeedMigration', () => this.#associateAccountKeysWithLegacySavedSeedMigration(accountPicker, keystore, onSuccess), true);
    }
    toJSON() {
        return {
            ...this,
