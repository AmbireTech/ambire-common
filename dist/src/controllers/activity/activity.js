...
const getPreviousBlockNumber = (blockNumber) => (blockNumber > 0 ? blockNumber - 1 : 0);
const normalizeTxnId = (txnId) => txnId?.toLowerCase();
/**
 * Take all txnIds from the account op
 * - normal case: accountOp.txnId
 * - MultipleTxns case: each call.txnId
 */
const getInternalAccountOpTxnIds = (accountOp) => {
    return [accountOp.txnId, ...accountOp.calls.map((call) => call.txnId)].filter((txnId) => !!txnId);
};
const internalAccountOpHasTxnId = (accountOp, txnId) => {
 ...
 */
const getAccountOpsAccountKey = (accountOps, accountAddr) => Object.keys(accountOps).find((key) => key.toLowerCase() === accountAddr.toLowerCase());
const getAccountOpsForAccountAndChain = (accountOps, accountAddr, chainIdString) => {
    const accountKey = getAccountOpsAccountKey(accountOps, accountAddr);
    return accountKey ? accountOps[accountKey]?.[chainIdString] || [] : [];
};
const getBalanceChangeWindowFromReceipts = (accountOp, receipts) => {
...
export class ActivityController extends EventEmitter {
    ...
    #onContractsDeployed;
    #callRelayer;
    #bannersByAccount = new Map();
    #updateAccountsOpsStatusesPromises = {};
    #backfillAccountOpBalanceChangesPromises = {};
    #addExternalAccountOpQueue = Promise.resolve();
    constructor(storage, fetch, callRelayer, accounts, selectedAccount, providers, networks, portfolio, safe, onContractsDeployed, eventEmitterRegistry) {
     ...
     */
    async hasAccountOpsSentTo(toAddress, // the address to check for received transactions
    accountId // the account ID to filter operations from
    ) {
        ...
        if (!found && bestPoisoningMatch) {
            const currentBestPoisoningMatch = bestPoisoningMatch;
    ...
    async filterAccountsOps(sessionId, filters, pagination = { fromPage: 0, itemsPerPage: 10 }) {
        await this.#initialLoadPromise;
        this.#externalAccountOps = await this.#storage.get('externalAccountOps', this.#externalAccountOps);
        ...
        const internalAccountOpsEntriesOnEnabledNetworks = Object.entries(internalAccountOpsByChain).filter(([chainId]) => enabledNetworkChainIds.includes(chainId));
        const internalAccountOps = new Set(internalAccountOpsEntriesOnEnabledNetworks.flatMap(([, accountOps]) => accountOps));
        ...
        if (filters.chainId && enabledNetworkChainIds.includes(String(filters.chainId))) {
            filteredItems = [
                ...(accountOpsEntriesOnEnabledNetworks.find(([chainId]) => chainId === String(filters.chainId))?.[1] || [])
            ];
        }
        else {
        ...
        if (filters.identifiedBy) {
            filteredItems = filteredItems.filter((i) => i.identifiedBy && i.identifiedBy.identifier === filters.identifiedBy.identifier);
        }
     ...
     */
    async #removeExternalAccountOpsMatchingInternalOps(accountOps) {
        let hasRemovedExternalAccountOps = false;
        accountOps.filter(isAccountOpFinalized).forEach((accountOp) => {
            const externalAccountOpsAccountKey = getAccountOpsAccountKey(this.#externalAccountOps, accountOp.accountAddr);
            if (!externalAccountOpsAccountKey)
                return;
            const chainIdString = accountOp.chainId.toString();
            const externalAccountOps = this.#externalAccountOps[externalAccountOpsAccountKey]?.[chainIdString];
            if (!externalAccountOps?.length)
                return;
            const internalTxnIds = new Set(getInternalAccountOpTxnIds(accountOp).map((txnId) => normalizeTxnId(txnId)));
            if (!internalTxnIds.size)
                return;
            ...
            if (filteredExternalAccountOps.length === externalAccountOps.length)
                return;
    ...
    async filterSignedMessages(sessionId, filters, pagination = { fromPage: 0, itemsPerPage: 10 }) {
        await this.#initialLoadPromise;
    ...
    async addExternalAccountOp({ accountAddr, chainId, txnId, receipt, callId, shouldLearnTokens = false }) {
        const task = this.#addExternalAccountOpQueue
            .catch(() => undefined) // errors handled inside
            .then(() => this.#addExternalAccountOp({
            accountAddr,
            chainId,
            txnId,
            receipt,
    ...
    async #addExternalAccountOp({ accountAddr, chainId, txnId, receipt, callId, shouldLearnTokens = false }) {
        ...
        const hasExistingAccountOpWithTxnId = () => {
            const internalAccountOps = getAccountOpsForAccountAndChain(this.#accountsOps, accountAddr, chainIdString);
            const existingExternalAccountOps = getAccountOpsForAccountAndChain(this.#externalAccountOps, accountAddr, chainIdString);
            return (internalAccountOps.some((accountOp) => internalAccountOpHasTxnId(accountOp, txnId)) ||
                existingExternalAccountOps.some((accountOp) => externalAccountOpHasTxnId(accountOp, txnId)));
        };
        if (hasExistingAccountOpWithTxnId())
            return;
        ...
        if (hasExistingAccountOpWithTxnId())
            return;
        if (!this.#externalAccountOps[accountAddr])
            this.#externalAccountOps[accountAddr] = {};
    ...
    async setAccountOpBalanceChanges(identifiedBy, accountAddr, chainId, balanceChanges) {
        await this.#initialLoadPromise;
     ...
     */
    async backfillAccountOpBalanceChangesAndPersist(accountOps) {
     ...
     * end of an operation
     */
    async backfillAccountOpBalanceChanges(accountOp) {
        await this.#initialLoadPromise;
        // take the latest #accountOp, not a stale one from the UI
        const currentAccountOp = this.findByIdentifiedBy(accountOp.identifiedBy, accountOp.accountAddr, accountOp.chainId);
        if (!currentAccountOp || typeof currentAccountOp.balanceChanges !== 'undefined')
            return;
        const taskId = `${accountOp.accountAddr}:${accountOp.chainId.toString()}:${accountOp.identifiedBy.identifier}`;
        if (this.#backfillAccountOpBalanceChangesPromises[taskId]) {
            return this.#backfillAccountOpBalanceChangesPromises[taskId];
        }
        this.#backfillAccountOpBalanceChangesPromises[taskId] = this.#prepareAndRunBalanceChangesTask(currentAccountOp).finally(() => {
    ...
    async #prepareAndRunBalanceChangesTask(accountOp) {
        ...
        try {
            ...
            if (!receipts.length) {
                await this.setAccountOpBalanceChanges(accountOp.identifiedBy, accountOp.accountAddr, accountOp.chainId, new Error('no receipts found'));
                return;
            }
            ...
            await this.updateAccountOpBalanceChanges(accountOp, network, tokenAddrs, balanceChangeWindow.receiptBlockNumber, balanceChangeWindow.prevBlockNumber, receipts);
        }
        catch (error) {
            console.log(error);
            await this.setAccountOpBalanceChanges(accountOp.identifiedBy, accountOp.accountAddr, accountOp.chainId, error);
        }
    }
    async updateAccountsOpsStatuses(accountAddresses = []) {
        ...
        const uniqueAddresses = Array.from(new Set([...accountAddresses, selectedAddr].filter(Boolean)));
        const results = await Promise.all(uniqueAddresses.map(async (addr) => {
            if (this.#updateAccountsOpsStatusesPromises[addr])
                return [addr, await this.#updateAccountsOpsStatusesPromises[addr]];
            this.#updateAccountsOpsStatusesPromises[addr] = this.#updateAccountsOpsStatuses(addr).finally(() => {
    ...
    async #executeBalanceChanges(balanceChangesTasks) {
        await Promise.all(balanceChangesTasks.map(({ accountOp, network, tokenAddrs, receiptBlockNumber, prevBlockNumber, receipts }) => this.updateAccountOpBalanceChanges(accountOp, network, tokenAddrs, receiptBlockNumber, prevBlockNumber, receipts)));
        await this.persistAccountsOps();
    }
    /**
     ...
     */
    async #updateAccountsOpsStatuses(accountAddr) {
        await this.#initialLoadPromise;
        if (!this.#accountsOps[accountAddr])
            return {
        ...
        await Promise.all(Object.keys(this.#accountsOps[accountAddr]).map(async (keyAsChainId) => {
            ...
            return Promise.all(opsToUpdate.map(async (accountOp) => {
                ...
                let lastReceiptBlockNumber;
                let shouldScheduleBalanceChangesTask = false;
                const foundTokensForBalanceChanges = new Set();
                const receiptsForBalanceChanges = [];
                ...
                const hasConfirmedOpWithSameEoaNonce = accountOp.eoaNonce !== null &&
                    ...
                    for (let i = 0; i < receipts.length; i++) {
                        ...
                        if (receipt) {
                            ...
                            if (!isSuccess) {
                                ...
                                const acc = this.#accounts.accounts.find((a) => a.addr === accountOp.accountAddr);
                                if (acc && !!acc.safeCreation) {
    ...
    async updateAccountOpBalanceChanges(accountOp, network, tokenAddrs, receiptBlockNumber, prevBlockNumber, receipts) {
        await this.#initialLoadPromise;
        try {
    ...
    get broadcastedButNotConfirmed() {
        return Object.fromEntries(this.#accounts.accounts.map((acc) => {
    ...
    findByIdentifiedBy(identifiedBy, accountAddr, chainId) {
        ...
        return this.#accountsOps[accountAddr]?.[chainId.toString()]?.find((op) => op.identifiedBy && op.identifiedBy.identifier === identifiedBy.identifier);
    }
    get banners() {
        ...
        const mapToMetaData = (ops) => ops.map((op) => ({
        ...
        for (const acc of this.#accounts.accounts) {
            ...
            const pendingOpsWithUpdatedStatus = pendingBanner
                ? latestOps.filter((op) => pendingBanner.meta.accountOpsDataForNextUpdate.find((meta) => meta.accountAddr === op.accountAddr &&
    ...
    getAccountOpsForAccount({ accountAddr = this.#selectedAccount.account?.addr, from, numberOfItems, 
    ...
    sortAccOps = true }) {
        ...
        if (sortAccOps)
            allAccountOps = allAccountOps.sort((a, b) => b.timestamp - a.timestamp);
