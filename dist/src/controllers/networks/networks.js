...
const networks_3 = require("../../utils/networks");
...
class NetworksController extends eventEmitter_1.default {
    ...
    initialLoadPromise;
    constructor(storage, fetch, relayerUrl, onAddOrUpdateNetwork, onRemoveNetwork) {
        super();
    ...
    async #load() {
        ...
        const networksInStorage = await this.#storage.get('networks', {});
        let finalNetworks = {};
        // If networksInStorage is empty, set predefinedNetworks and emit update
        if (!Object.keys(networksInStorage).length) {
            finalNetworks = networks_1.networks.reduce((acc, network) => {
     ...
     *
     */
    async mergeRelayerNetworks(finalNetworks, networksInStorage) {
        let relayerNetworks = {};
        ...
        try {
            ...
            Object.entries(relayerNetworks).forEach(([_chainId, network]) => {
                ...
                const relayerNetwork = (0, networks_3.mapRelayerNetworkConfigToAmbireNetwork)(chainId, network);
            ...
            Object.keys(updatedNetworks).forEach((chainId) => {
    ...
    async #updateNetworkFeatures(finalNetworks) {
        const updatePromises = Object.values(finalNetworks).map(async (network) => {
            ...
            if (network.lastUpdatedNetworkInfo &&
                ...
                return;
            // eslint-disable-next-line @typescript-eslint/no-floating-promises
            (0, networks_2.getNetworkInfo)(this.#fetch, network.selectedRpcUrl, network.chainId, async (info) => {
    ...
    async #updateNetwork(network, chainId) {
        ...
        const checkRPC = async (networkToAddOrUpdate) => {
            if (changedNetwork.selectedRpcUrl) {
                ...
                // eslint-disable-next-line @typescript-eslint/no-floating-promises
                (0, networks_2.getNetworkInfo)(this.#fetch, changedNetwork.selectedRpcUrl, this.#networks[chainId.toString()].chainId, async (info) => {
                    if (Object.values(info).some((prop) => prop === 'LOADING')) {
                        return;
                    }
                    const { feeOptions } = info;
