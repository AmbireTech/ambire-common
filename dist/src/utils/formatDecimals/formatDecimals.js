 ...
 */
const removeTrailingZeros = (decimalStr, minDecimals = 0) => {
    if (!decimalStr.includes('.'))
        return decimalStr; // If there's no decimal point, return the original string
...
const getIndexOfFirstNonZeroInDecimals = (value, type) => {
...
const getPrefix = (widthDollarPrefix) => (widthDollarPrefix ? '$' : '');
const formatNumber = (value, withDollarPrefix, decimals, sign, type) => {
    ...
    const stringValueWithoutTrailingZeros = removeTrailingZeros(reconstructedStringValue, type ? DECIMAL_RULES[type].min : undefined);
...
const formatDecimals = (value = undefined, type = 'default') => {
    const withDollarPrefix = TYPES_WITH_DOLLAR_PREFIX.includes(type || '');
    if (value === 0) {
        if (type === 'amount')
            return `${getPrefix(withDollarPrefix)}0`;
        return `${getPrefix(withDollarPrefix)}0.00`;
    }
    if (!value || Number.isNaN(value))
        return `${getPrefix(withDollarPrefix)}-`;
