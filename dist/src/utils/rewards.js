...
const calculateRewardsStats = (projectedRewardsResult, walletOrStkWalletTokenPrice, currentBalance, stkBalanceUsd, walletEthProvidedLiquidityInUsd) => {
    ...
    const { weeksWithData, governanceVotes, rank, poolSize, pointsOfOtherUsers, swapVolume } = projectedRewardsResult;
    const { sumBalanceSnapshots, sumLiquiditySnapshots, sumStkBalanceSnapshots } = weeksWithData.reduce((acc, week) => {
    ...
    const numberOfWeeksSinceStartOfSeason = projectedRewardsResult.numberOfWeeksSinceStartOfSeason || 1;
    const seasonAverageBalance = typeof currentBalance !== 'undefined'
        ? (sumBalanceSnapshots + currentBalance) / (numberOfWeeksSinceStartOfSeason + 1)
        : sumBalanceSnapshots / numberOfWeeksSinceStartOfSeason;
    const seasonLiquidityAverage = typeof walletEthProvidedLiquidityInUsd !== 'undefined'
    ...
    const seasonStkWalletBalanceAverage = typeof stkBalanceUsd !== 'undefined'
    ...
    if (!reasonToNotDisplayProjectedRewards)
        totalScore = Math.floor((balanceScore + stkWALLETScore + liquidityScore + swapVolumeScore + governanceScore) *
            totalMultiplier);
    const estimatedRewardsUSD = Math.floor((totalScore / (pointsOfOtherUsers + totalScore)) * poolSize);
