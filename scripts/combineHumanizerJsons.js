...
const amireConstants = path.join(__dirname, '..', 'src', 'consts', 'ambireConstantsHumanizerInfo.json')
...
const main = async () => {
	const prepedJsons = await Promise.all([
		...
		fsPromises.readFile(amireConstants, 'utf8').then(JSON.parse).then(initHumanizerMeta)
	])
