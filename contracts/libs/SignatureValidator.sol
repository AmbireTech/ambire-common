...
library SignatureValidator {
	...
	enum SignatureMode {
		...
		Schnorr,
		Multisig,
		// MerkleTree is legit but validation is handled in the contract itself.
		// if it comes to the SignatureValidator, it should revert.
		// Nevertheless, we should have it here to not mix modes
		MerkleTree,
	...
	function getSignatureMode(bytes memory sig) internal pure returns (SignatureMode mode) {
		...
		mode = SignatureMode(modeRaw);
	}

	function recoverAddrAllowUnprotected(bytes32 hash, bytes memory sig, bool allowSpoofing) internal view returns (address, bool) {
		SignatureMode mode = getSignatureMode(sig);
		return recoverAddrAllowUnprotectedWithMode(hash, sig, allowSpoofing, mode);
	}

	function recoverAddrAllowUnprotectedWithMode(bytes32 hash, bytes memory sig, bool allowSpoofing, SignatureMode mode) internal view returns (address, bool) {
