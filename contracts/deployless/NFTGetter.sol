...
contract NFTGetter is Simulation  {
	...
	function getCollectionMeta(IAmbireAccount account, NFT collection, uint[] memory tokenIds, uint limit) external view returns (NFTCollectionMetadata memory meta) {
	...
	function getAllNFTs(IAmbireAccount account, NFT[] memory collections, uint[][] memory tokenIds, uint tokenPerCollectionLimit) public view returns (NFTCollectionMetadata[] memory) {
		uint len = collections.length;
		NFTCollectionMetadata[] memory collectionMetas = new NFTCollectionMetadata[](len);
		for (uint i=0; i!=len; i++) {
			try this.getCollectionMeta{ gas: 15000*tokenPerCollectionLimit }(account, collections[i], tokenIds[i], tokenPerCollectionLimit)
			returns (NFTCollectionMetadata memory meta) { collectionMetas[i] = meta; }
			catch (bytes memory err) { collectionMetas[i].error = err.length == 0 ? bytes('REVERT') : err; }
		}
	...
	function simulateAndGetAllNFTs(
		...
		NFTCollectionAtNonce memory before, NFTCollectionAtNonce memory afterSimulation,
		...
		if (afterSimulation.nonce != before.nonce) {
			afterSimulation.collections = getAllNFTs(account, collections, tokenIds, tokenPerCollectionLimit);
		}
