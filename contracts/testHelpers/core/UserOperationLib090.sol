...
library UserOperationLib090 {
  ...
  uint256 internal constant MIN_PAYMASTER_DATA_WITH_SUFFIX_LEN =
    PAYMASTER_DATA_OFFSET + PAYMASTER_SUFFIX_LEN; // minimum length of paymasterData that can contain a paymaster signature.
  ...
  function unpackPaymasterVerificationGasLimit(
    ...
    return
      uint128(
        bytes16(
          userOp.paymasterAndData[PAYMASTER_VALIDATION_GAS_OFFSET:PAYMASTER_POSTOP_GAS_OFFSET]
        )
      );
  }
  ...
  function unpackPaymasterStaticFields(
    ...
    return (
      ...
      uint128(
        bytes16(paymasterAndData[PAYMASTER_VALIDATION_GAS_OFFSET:PAYMASTER_POSTOP_GAS_OFFSET])
      ),
  ...
  function getPaymasterSignatureLength(
    ...
    unchecked {
      ...
      uint256 pmSignatureLength = uint16(
        bytes2(paymasterAndData[dataLength - PAYMASTER_SUFFIX_LEN:])
      );
   ...
   */
  function getPaymasterSignature(
    bytes calldata paymasterAndData
  ...
  function getPaymasterSignatureWithLength(
    bytes calldata paymasterAndData,
    uint256 paymasterSignatureLength
  ) internal pure returns (bytes calldata pmSig) {
   ...
   */
  function encodePaymasterSignature(
