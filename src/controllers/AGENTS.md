...
- Do not delete the legacy key as part of a data migration while the IDB path is still new. Keep it as a safety-net copy and record a completion flag (e.g. `activityIdbMigrated`) instead. The controller must _read_ from that copy when the migration fails, otherwise it shows an empty state for the whole session while the data sits intact one key away. The flag itself has no consumer today — `ActivityController` writes it (`#recordMigrationCompleted` in `AccountOpsPersistence`) because it can only be recorded while IDB works, and a session that cannot open IDB can no longer tell "never had transactions" from "history is in IDB and unreachable". There is deliberately no banner or error for that state: it is detected inside `#load()` on service-worker startup, where errors reach nothing because no window is open, and a banner for a condition the user cannot act on was judged not worth the noise.
...
- **Two controllers use this now**: `ActivityController` (row-per-op, partial startup read) and `PhishingController` (single snapshot document, read whole). Phishing is the simpler shape to copy — one store, no indexes, no expansion — and its `phishing` store joined `AMBIRE_IDB_SCHEMA` at `dbVersion` 1 with no bump, because v1 had not shipped yet. Once a version ships, adding a store costs an unrollbackable bump, so land any store you already know you need before release.
...
- `AccountOpsPersistence` and `PhishingPersistence` are the two worked examples of the whole wiring. Copy the phishing one unless your data needs a partial startup read — it is the smaller shape.
