...
describe('Settings Controller', () => {
  ...
  test('should add the bnb network as a custom network', (done) => {
    settingsController.onUpdate(() => {
      const bnbNetwork = settingsController.networks.find(({ id }) => id === 'binance')
      expect(bnbNetwork).not.toBe(undefined)
      expect(bnbNetwork).not.toBe(null)
      expect(bnbNetwork?.chainId).toBe(56n)
      expect(bnbNetwork?.name).toBe('Binance')
      expect(bnbNetwork?.id).toBe('binance')
      expect(bnbNetwork?.nativeAssetSymbol).toBe('BNB')

      // fantom does not have the entry point
      expect(bnbNetwork?.erc4337?.enabled).toBe(false)
      expect(bnbNetwork?.erc4337?.hasPaymaster).toBe(false)

      // ...nor does it have the singleton
      expect(bnbNetwork?.isSAEnabled).toBe(false)

      // so contracts are not deployed
      expect(bnbNetwork?.areContractsDeployed).toBe(false)

      // it is 1559
      expect(bnbNetwork?.feeOptions.is1559).toBe(false)

      // it is not optimistic
      expect(bnbNetwork?.isOptimistic).toBe(false)

      // simulation is somewhat supported
      expect(bnbNetwork?.rpcNoStateOverride).toBe(true)
      expect(bnbNetwork?.hasDebugTraceCall).toBe(false)

      // contracts are not deployed
      const saSupport = bnbNetwork?.features.find((feat) => feat.id === 'flagged')
      expect(saSupport).not.toBe(null)
      expect(saSupport).not.toBe(undefined)
      expect(saSupport!.level).toBe('danger')
