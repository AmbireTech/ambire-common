...
describe('Portfolio', () => {
  ...
  test(`get aave positions for ${userAddrAave} on Optimism`, async () => {
    const aavePositions: PositionsByProvider | null = await getAAVEPositions(
      userAddrAave,
      providerOptimism,
      optimism
    )
  ...
  test(`get uni v3 positions for ${userAddrUni} on Polygon`, async () => {
    const uniV3Positions: PositionsByProvider | null = await getUniV3Positions(
      userAddrUni,
      providerPolygon,
      polygon
    )
    ...
    if (uniV3Positions !== null) {
      expect(uniV3Positions.positions[0].additionalData.inRange).toBe(false)
    }
  })
})
