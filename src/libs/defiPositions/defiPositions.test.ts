...
describe('Portfolio', () => {
    ...
    test(`get aave positions for ${userAddrAave} on Optimism`, async () => {
        const aavePositions: Positions[] | null = await getAAVEPositions(userAddrAave, providerOptimism, optimism)
    ...
    test(`get uni v3 positions for ${userAddrUni} on Polygon`, async () => {
        const uniV3Positions: Positions[] | null = await getUniV3Positions(userAddrUni, providerPolygon, polygon)
