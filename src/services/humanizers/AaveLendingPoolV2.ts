...
const AaveMapping = (humanizerInfo:HumanizerInfoType) => {
  ...
  return {
    [iface.getSighash('deposit')]: (txn, network, { extended }) => {
      ...
      if (extended) return toExtended('Deposit', 'to', humanizerInfo, asset, amount, true, txn, onBehalf)
      return [
        `Deposit ${token(humanizerInfo, asset, amount)} to Aave lending pool${onBehalfText(onBehalf, txn.from)}`
      ]
    },
    [iface.getSighash('withdraw')]: (txn, network, { extended }) => {
      ...
      return [
        `Withdraw ${token(humanizerInfo, asset, amount)} from Aave lending pool${onBehalfText(onBehalf, txn.from)}`
      ]
    },
