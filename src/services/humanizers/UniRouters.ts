  ...
  return {
    ...
    [iface.getSighash('addLiquidityETH')]: (txn, network, opts = {}) => {
      ...
      const [token, amountTokenDesired /* amountTokenMin */ /* amountETHMin */, , , to, deadline] =
        args
      return [
        `Add liquidity: ${token(humanizerInfo, token, amountTokenDesired)} and ${nativeToken(
          network,
          value
        )}${recipientText(to, txn.from)}${deadlineText(deadline, opts.mined)}`
      ]
    ...
    [ifaceV3.getSighash('multicall')]: (txn, network) => {
      ...
      const parsed = calls
        .map((data) => {
          ...
          const humanizer = uniV3Mapping(humanizerInfo)[sigHash]
  ...
  return {
    [ifaceV32.getSighash('multicall(uint256,bytes[])')]: (txn, network, opts = {}) => {
      ...
      const parsed = calls
        .map((data) => {
          const sigHash = data.slice(0, 10)
          const humanizer = uniV32Mapping(humanizerInfo)[sigHash]
          return humanizer ? humanizer({ ...txn, data }, network) : null
        })
        .flat()
        .filter((x) => x)
