...
const recipientText = (recipient, txnFrom, extended = false) =>
  recipient.toLowerCase() === txnFrom.toLowerCase()
    ...
    : ['and send it to', { type: 'address', address: recipient }]
...
const uniV2Mapping = {
  ...
  [iface.getSighash('swapExactTokensForTokens')]: (txn, network, opts = {}) => {
    ...
    return !opts.extended
      ? [
          `Swap ${token(path[0], amountIn)} for at least ${token(
            outputAsset,
            amountOutMin
          )}${recipientText(to, txn.from)}${deadlineText(deadline, opts.mined)}`
        ]
      : toExtended(
          token(path[0], amountIn, true),
          token(outputAsset, amountOutMin, true),
          recipientText(to, txn.from, true),
  ...
  [iface.getSighash('swapTokensForExactTokens')]: (txn, network, opts = {}) => {
    ...
    return !opts.extended
      ? [
          `Swap up to ${token(path[0], amountInMax)} for ${token(
            outputAsset,
            amountOut
          )}${recipientText(to, txn.from)}${deadlineText(deadline, opts.mined)}`
        ]
      : toExtended(
          token(path[0], amountInMax, true),
          token(outputAsset, amountOut, true),
          recipientText(to, txn.from, true),
          deadlineText(deadline, opts.mined),
          false
        )
  ...
  [iface.getSighash('swapTokensForExactETH')]: (txn, network, opts = {}) => {
    ...
    return !opts.extended
      ? [
          `Swap up to ${token(path[0], amountInMax)} for ${nativeToken(
            network,
            amountOut
          )}${recipientText(to, txn.from)}${deadlineText(deadline, opts.mined)}`
        ]
  ...
  [iface.getSighash('swapExactTokensForETH')]: (txn, network, opts = {}) => {
    ...
    return !opts.extended
      ? [
          `Swap ${token(path[0], amountIn)} for at least ${nativeToken(
            network,
            amountOutMin
          )}${recipientText(to, txn.from)}${deadlineText(deadline, opts.mined)}`
        ]
  ...
  [iface.getSighash('addLiquidity')]: (txn, network, opts = {}) => {
    ...
    return [
      `Add liquidity: ${token(tokenA, amountADesired)} and ${token(
        tokenB,
        amountBDesired
      )}${recipientText(to, txn.from)}${deadlineText(deadline, opts.mined)}`
    ]
  ...
  [iface.getSighash('addLiquidityETH')]: (txn, network, opts = {}) => {
    ...
    const [token, amountTokenDesired /* amountTokenMin */ /* amountETHMin */, , , to, deadline] =
      args
    return [
      `Add liquidity: ${token(token, amountTokenDesired)} and ${nativeToken(
        network,
        value
      )}${recipientText(to, txn.from)}${deadlineText(deadline, opts.mined)}`
    ]
  ...
  [iface.getSighash('removeLiquidity')]: (txn, network, opts = {}) => {
    const [tokenA, tokenB /* liquidity */, , amountAMin, amountBMin, to, deadline] =
      iface.parseTransaction(txn).args
    return [
      `Remove liquidity: at least ${token(tokenA, amountAMin)} and ${token(
        tokenB,
        amountBMin
      )}${recipientText(to, txn.from)}${deadlineText(deadline, opts.mined)}`
    ]
  ...
  [iface.getSighash('removeLiquidityETH')]: (txn, network, opts = {}) => {
    const [token /* liquidity */, , amountTokenMin, amountETHMin, to, deadline] =
      iface.parseTransaction(txn).args
    return [
      `Remove liquidity: at least ${token(token, amountTokenMin)} and ${nativeToken(
        network,
        amountETHMin
      )}${recipientText(to, txn.from)}${deadlineText(deadline, opts.mined)}`
    ]
...
const uniV3Mapping = {
  ...
  [ifaceV3.getSighash('exactInputSingle')]: (txn, network, opts = {}) => {
    ...
    // @TODO: consider fees
    return [
      `Swap ${token(params.tokenIn, params.amountIn)} for at least ${token(
        params.tokenOut,
  ...
  [ifaceV3.getSighash('exactInput')]: (txn, network, opts = {}) => {
    ...
    return [
      `Swap ${token(path[0], params.amountIn)} for at least ${token(
        path[path.length - 1],
        params.amountOutMinimum
      )}${recipientText(params.recipient, txn.from)}${deadlineText(params.deadline, opts.mined)}`
    ]
  ...
  [ifaceV3.getSighash('exactOutputSingle')]: (txn, network, opts = {}) => {
    const [params] = ifaceV3.parseTransaction(txn).args
    return [
      `Swap up to ${token(params.tokenIn, params.amountInMaximum)} for ${token(
        params.tokenOut,
        params.amountOut
      )}${recipientText(params.recipient, txn.from)}${deadlineText(params.deadline, opts.mined)}`
    ]
  ...
  [ifaceV3.getSighash('exactOutput')]: (txn, network, opts = {}) => {
    ...
    const path = parsePath(params.path)
    return [
      `Swap up to ${token(path[0], params.amountInMaximum)} for ${token(
        path[path.length - 1],
        params.amountOut
      )}${recipientText(params.recipient, txn.from)}${deadlineText(params.deadline, opts.mined)}`
    ]
  ...
  [ifaceV3.getSighash('unwrapWETH9')]: (txn, network) => {
    ...
    return [
      `Unwrap at least ${nativeToken(network, amountMin)}${recipientText(recipient, txn.from)}`
    ]
  }
}
