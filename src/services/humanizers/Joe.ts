...
import { token, getName, nativeToken } from '../humanReadableTransactions'
    ...
    [iface.getSighash('swapExactTokensForAVAX')]: (txn, network, { extended }) => {
      ...
      if (extended)
        return [
          [
            'Swap',
            { type: 'token', ...(token(humanizerInfo, path[0], amountIn, true) as object) },
            'for at least',
            {
              type: 'token',
              ...(token(humanizerInfo, path[path.length - 1], amountOutMin, true) as object)
            },
            ...(txn.from !== to
              ? [
                  'and send swapped to',
