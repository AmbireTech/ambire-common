...
import { Connection, Request, UseWalletConnectProps, UseWalletConnectReturnType } from './types'
...
const UNISWAP_PERMIT_EXCEPTIONS = [
  // based on PeerMeta
...
const checkIsOffline = (uri: string) => {
...
export default function useWalletConnect({
  ...
  useStorage,
  clearWcClipboard
}: UseWalletConnectProps): UseWalletConnectReturnType {
  const { addToast } = useToasts()
  const { selectedAcc: account } = useAccounts()
  const { network, allNetworks, setNetwork } = useNetwork()
  ...
  const [state, dispatch] = useReducer(
    ...
    (state: any, action: any) => {
      ...
      if (action.type === 'batchRequestsAdded') {
        if (state.requests.find(({ id }) => id === `${action.batchRequest.id}:0`))
          return { ...state }
      ...
      if (action.type === 'requestAdded') {
        if (state.requests.find(({ id }: Request) => id === action.request.id)) return { ...state }
        return { ...state, requests: [...state.requests, action.request] }
      }
      if (action.type === 'requestsResolved') {
        return {
          ...state,
          requests: state.requests.filter((x: Request) => !action.ids.includes(x.id))
        }
      }
      return { ...state }
    },
  ...
  const maybeUpdateSessions = () => {
    ...
    state.connections.forEach(({ uri, isOffline }: Connection) => {
      if (connectors[uri]) {
  ...
  const connect = useCallback(
    async (connectorOpts: any) => {
      ...
      connector.on('session_request', async (error, payload) => {
        ...
        !!clearWcClipboard && clearWcClipboard()
  ...
  const disconnect = useCallback((uri: string) => {
  ...
  const resolveMany = (ids: string[], resolution: any) => {
    state.requests.forEach(({ id, wcUri, isBatch }: Request) => {
      if (ids.includes(id)) {
        const connector = connectors[wcUri]
        if (!connector) return
        if (!isBatch || id.endsWith(':0')) {
          const realId = isBatch ? id.substr(0, id.lastIndexOf(':')) : id
