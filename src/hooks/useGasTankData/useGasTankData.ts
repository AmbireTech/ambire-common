      ...
      feeAssetsPerNetwork?.map((item: any) => {
        ...
        if (isFound)
          return {
            ...
            balanceUSD:
              parseFloat(isFound?.balance?.toString() || '0') *
              parseFloat(
    ...
    () =>
      ...
      gasTankTxns.map((item: any) => {
        const txnFilled =
          ...
          gasTankFilledTxns.find((i: any) => i._id === item._id)
        const feeToken =
          (item.feeToken && item.feeToken.symbol && item.feeToken.symbol.toLowerCase()) ||
          item.feeToken ||
          (txnFilled &&
            feeAssetsRes.find(
              (i: any) =>
                i.network === network?.id &&
                i.address.toLowerCase() === txnFilled.address.toLowerCase()
            )?.symbol)
